...
// Edit the following to adjust the calibration parameters
	float parIntercept = -0.590277;
	float parSlope = 0.00536341;
	...
	*/
	...
	int binNum = int ( len*parSlope) * 50;

	TH1F *hcal = new TH1F("hcal" , "h with calibration" , len , parIntercept , (len- 1)*parSlope + parIntercept);
